...
Postmanコレクションは、期限が切れた`access_token`値を自動的に検出し、`refresh_token`を使用して新しい値をリクエストできます。デフォルトでは、この機能は無効になっていますが、Postman環境変数`enable_auto_refresh_access_token`を`true`に設定することで有効にできます。
