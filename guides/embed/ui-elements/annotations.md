...
ここで、`disabledAnnotationTypes`は、無効にする有効な注釈の種類の文字列です。ビューアー固有の注釈の構成の詳細については、以下の「[注釈の有効化/無効化と注釈の種類](#section-enabling-disabling-annotations-and-annotation-types)」を参照してください。
...
| `saveAnnotation`   | Saves an annotation locally and on the server   | {string} 注釈の種類、{text} 保存する注釈のテキスト                  |
| `deleteAnnotation` | Deletes an annotation                           | {string} 注釈ID、{boolean} サーバー上で削除するかどうか(デフォルトはtrue) |
...
| `addAnnotation`    | Adds an annotation to the dialog          | {Annotation} 追加する注釈 |
| `removeAnnotation` | Removes an annotation from the dialog     | {string} 注釈ID       |
| `postAnnotation`   | Posts an annotation in the dialog         | {string} 投稿する注釈テキスト |
